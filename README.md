...
### 変更後（クライアントワーク）のページ
[変更後（クライアントワーク）のページ](https://www.mikuro.works/portfolio/works/experience_based_operation_explanation/)
...
| Webサーバーの環境（自前・レンタル・有料・無料） | 自前を想定（提出にはリソース・書類のみで、実装は顧客が行う）<br>[確認環境](#)は提供する |
